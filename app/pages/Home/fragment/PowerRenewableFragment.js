...
type Props = {};
export default class PowerRenewableFragment extends Component<Props> {
    constructor(props) {
        super(props);
        this.state = {
            option: {},
    ...
    _sendRequest() {
        let title = '当月可再生能源发电量';
        storage.load({key: 'loginState'})
            .then((loginState) => {
                Api.setToken(loginState.token)
                    .get('/api/home/load_generated')
                    .then((data) => {
                        let unit = data.ratio + loginState.unit.power;
                        this.setState({
                            isLoading: false,
