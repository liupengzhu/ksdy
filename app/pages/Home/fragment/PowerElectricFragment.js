...
type Props = {};
export default class PowerElectricFragment extends Component<Props> {
    constructor(props) {
        super(props);
        this.state = {
            option: {},
    ...
    _sendRequest() {
        let title = '24小时内用电负荷曲线';
        storage.load({key: 'loginState'})
            .then((loginState) => {
                let unit = loginState.unit.load;
                Api.setToken(loginState.token)
                    .get('/api/home/load_usage')
                    .then((data) => {
                        this.setState({
                            isLoading: false,
                                        ...
                                        for (var i = 0; i < params.length; i++) {
                                            if (params[i].data != null) {
                                                res += '<p style="margin: 0px"><span style="color:' + params[i].color
                                                    ...
                                                    + params[i].data.value.toFixed(2) + params[i].data.name
                                                    + '</span></p>';
