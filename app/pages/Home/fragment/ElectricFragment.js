...
type Props = {};
export default class ElectricFragment extends Component<Props> {
    constructor(props) {
        super(props);
        this.state = {
            showPie: false,
    ...
    _renderItem({item}) {
        let view = this.state.date_type === 2 ? <ElectricDayItem data={item} ratio={this.state.tableRatio}/> :
            <ElectricItem data={item} ratio={this.state.tableRatio}/>
        return (
            <View>
                {view}
            </View>
    ...
    _sendRequest(data) {
        ...
        let title = data.date_type === 0 ? `${moment(data.date).format("YYYY年")} 区间用电量柱状图` : data.date_type === 1
            ? `${moment(data.date).format("YYYY年MM月")} 区间用电量柱状图`
            : `${moment(data.date).format("YYYY年MM月DD日")} 区间用电量曲线图`;
        let isFold = data.type === 1;
        storage
            .load({key: 'loginState'})
            .then((loginState) => {
                Api.setToken(loginState.token)
                    .get('/api/power/usage', {date_type, type, time: data.date})
                    .then((data) => {
                        let ratio = isFold ? 'tce'
                            : `${data.ratio ? data.ratio : data.chart_ratio}${loginState.unit.power}`;
                            ...
                            [
                                {
                                    ...
                                    type: 'bar',
                                    stack: '用电',
                                    data: data.chart.valley.map((item) => {
                                        return {
                                            value: item,
                                            name: ratio
                                        };
                                ...
                                },
                                {
                                    ...
                                    type: 'bar',
                                    stack: '用电',
                                    data: data.chart.normal.map((item) => {
                                        return {
                                            value: item,
                                            name: ratio
                                        };
