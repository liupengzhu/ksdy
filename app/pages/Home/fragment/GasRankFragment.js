...
import GasRankItem from "../../../Component/item/GasRankItem";
import LoadingView from "../../../Component/LoadingView";

type Props = {};
export default class GasRankFragment extends Component<Props> {
    constructor(props) {
        super(props);
        this.state = {
            date_type: 1,
    ...
    _sendRequest(data) {
        ...
        let title = data.date_type === 0 ? `${moment(data.date).format("YYYY年")} ${titleType}耗气排行占比图` : data.date_type === 1
            ? `${moment(data.date).format("YYYY年MM月")} ${titleType}耗气排行占比图`
            : `${moment(data.date).format("YYYY年MM月DD日")} ${titleType}耗气排行占比图`;
        ...
        storage
            ...
            .then((loginState) => {
                Api.setToken(loginState.token)
                    .get(`/api/gas/rank_${data.rank_type}`, {date_type, type, time: data.date})
                    .then((data) => {
                        this.setState({
                            ...
                            ratio: isFold ? 'tce' : `${loginState.unit.gas_usage}`,
                            isLoading: false,
                        })
                        ;
                    })
