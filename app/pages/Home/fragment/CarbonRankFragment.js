...
import WaterRankItem from "../../../Component/item/WaterRankItem";
import CarbonRankItem from "../../../Component/item/CarbonRankItem";
import LoadingView from "../../../Component/LoadingView";

type Props = {};
export default class CarbonRankFragment extends Component<Props> {
    constructor(props) {
        super(props);
        this.state = {
            date_type: 1,
    ...
    _sendRequest(data) {
        ...
        let title = data.date_type === 0 ? `${moment(data.date).format("YYYY年")} ${titleType}二氧化碳排放排行占比图` : data.date_type === 1
            ? `${moment(data.date).format("YYYY年MM月")} ${titleType}二氧化碳排放排行占比图`
            : `${moment(data.date).format("YYYY年MM月DD日")} ${titleType}二氧化碳排放排行占比图`;
        let series_name = data.date_type === 0 ? '当年排放' : data.date_type === 1 ? '当月排放' : '当日排放';
        storage
            .load({key: 'loginState'})
            .then((loginState) => {
                Api.setToken(loginState.token)
                    .get(`/api/carbon/rank_${data.rank_type}`, {date_type, type, time: data.date})
                    .then((data) => {
                        this.setState({
                            ...
                            ratio: `${loginState.unit.carbon_emissions}`,
                            isLoading: false,
                        });
                    })
                    .catch((error) => {
