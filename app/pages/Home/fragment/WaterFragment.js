...
type Props = {};
export default class WaterFragment extends Component<Props> {
    constructor(props) {
        super(props);
        this.state = {
            date_type: 1,
    ...
    _renderItem({item}) {
        return (
            <WaterItem data={item} ratio={this.state.tableRatio}/>
        );
    }
    ...
    _sendRequest(data) {
        ...
        let title = data.date_type === 0 ? `${moment(data.date).format("YYYY年")} 区间用水量柱状图` :
            `${moment(data.date).format("YYYY年MM月")} 区间用水量柱状图`
        let isFold = data.type === 1;
        storage
            .load({key: 'loginState'})
            .then((loginState) => {
                Api.setToken(loginState.token)
                    .get('/api/water/usage', {date_type, type, time: data.date})
                    .then((data) => {
                        let ratio = isFold ? 'tce'
                            : `${loginState.unit.water_usage}`;
                        let barData = [
                            {
                                name: '用水量',
                                type: 'bar',
