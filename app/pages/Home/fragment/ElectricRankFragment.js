...
import ElectricRankItem from "../../../Component/item/ElectricRankItem";
import RankTypeView from "../../../Component/RankTypeView";
import ChartEmptyView from "../../../Component/ChartEmptyView";
import LoadingView from "../../../Component/LoadingView";

type Props = {};
export default class ElectricRankFragment extends Component<Props> {
    constructor(props) {
        super(props);
        this.state = {
            date_type: 1,
    ...
    _sendRequest(data) {
        ...
        let title = data.date_type === 0 ? `${moment(data.date).format("YYYY年")} ${titleType}耗电排行占比图` : data.date_type === 1
            ? `${moment(data.date).format("YYYY年MM月")} ${titleType}耗电排行占比图`
            : `${moment(data.date).format("YYYY年MM月DD日")} ${titleType}耗电排行占比图`;
        ...
        storage
            ...
            .then((loginState) => {
                Api.setToken(loginState.token)
                    .get(`/api/power/rank_${data.rank_type}`, {date_type, type, time: data.date})
                    .then((data) => {
                        let ratio = isFold ? 'tce' : `${data.chart.ratio}${loginState.unit.power}`;
                        this.setState({
